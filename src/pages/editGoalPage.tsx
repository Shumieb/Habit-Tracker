...
function EditGoalPage() {
  ...
  useEffect(() => {
    ...
    if (foundHabit) {
      ...
      setCategoryId(foundHabit.category);
      setFrequencyId(foundHabit.frequency);

      if (foundHabit.frequency == "2") {
        setDayOfWeek(foundHabit.day);
      }
  ...
  return (
    <section className="w-[90%] mx-auto py-2 text-purple-950">
      <h1 className="text-xl px-2 py-2 text-center">Edit Goal</h1>

      <form onSubmit={(e) => handleSubmit(e)} className="w-[60%] mx-auto">
        {/* goal name */}
        <div className="py-2 px-2">
          <label className="text-lg px-2">Goal:</label>
          <input
            type="text"
            name="goal"
